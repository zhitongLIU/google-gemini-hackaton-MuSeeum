...
function parseCandidateFromText(text: string): ArtInfoCandidate {
  ...
  let confidence: ArtInfoCandidate["confidence"] = "medium";
  if (t.includes("high confidence") || t.includes("confident")) confidence = "high";
  else if (t.includes("low confidence") || t.includes("uncertain")) confidence = "low";
  return { title, artist, museum, year, period, confidence };
}

export async function runArtInfoAgent(
  apiKey: string,
  imageBase64: string,
  mimeType = "image/jpeg"
): Promise<{ candidate: ArtInfoCandidate; rawText?: string }> {
...
If you cannot identify the work, give your best guess and set Confidence to low. Be concise.`;
  ...
  if (!text) throw new Error("Art Info Agent returned no text");
